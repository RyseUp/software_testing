...
public class Process_Sach {
	...
	public static Process_Sach getInstance(){
		return new Process_Sach();
	}
	...
	public ArrayList<Sach> getByCondition(String condition)
	{
		Connection cn = cd.getCon();
		ArrayList<Sach> ls = new ArrayList<>();
		String sql = "SELECT * FROM tb_sach WHERE "+condition;
		try {
			PreparedStatement ps = (PreparedStatement) cn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
